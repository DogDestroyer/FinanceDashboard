...
/* ---------------- Risk ---------------- */
export function Risk({ valued, cash, cashUSD, navUSD, fx, hist, state, onAddTrade }: {
  valued: Valued[]; cash: Record<string, number>; cashUSD: number; navUSD: number;
  fx: FxMap; hist: any; state: AppState; onAddTrade: () => void;
}) {
  const stats = useMemo(() => {
  ...
  const Stat = ({ label, value, note }: { label: string; value: string; note: string }) => (
    <div className="card">
      ...
      <p className="num text-2xl mt-1.5">{value}</p>
      <p className="t-caption mt-1.5">{note}</p>
    </div>
  );

  if (!state.transactions.length) return <EmptyState text="Risk analytics need price history. Log a trade to start the clock." actionLabel="+ Trade" onAction={onAddTrade} />;
  ...
  return (
    <>
      ...
      {riskCharts && riskCharts.volCount >= 2 && (
        <div className="card">
          <h2 className="t-title mb-1">Rolling volatility, 30 day</h2>
          <p className="t-caption mb-2">Annualized standard deviation of the trailing 30 days of daily TWR. A rising line means the book is getting choppier, independent of direction.</p>
          <TSChart series={riskCharts.vol} height={180} valueFmt={pctFmt} />
        </div>
      )}
      {riskCharts && stats && stats.idx.length >= 2 && (
        <div className="card">
          <h2 className="t-title mb-1">Drawdown</h2>
          <p className="t-caption mb-2">Distance of the TWR index below its running peak. Flat at zero means you are at a high-water mark; the troughs are what you had to sit through.</p>
          <TSChart series={riskCharts.dd} height={180} valueFmt={pctFmt} />
        </div>
      )}
      <div className="card">
        <h2 className="t-title mb-1">Currency exposure</h2>
        <p className="t-caption mb-3">Where your NAV actually lives. For an SGD-based investor, USD assets carry FX risk on top of asset risk.</p>
        {ccyExposure.map(([c, v]) => (
          <div key={c} className="flex items-center gap-2 text-xs mb-2">
            <span className="num w-10 text-fog">{c}</span>
            <div className="flex-1 h-2 bg-ink rounded-full overflow-hidden">
              <div className="h-full bg-brass rounded-full" style={{ width: `${navUSD ? (v / navUSD) * 100 : 0}%` }} />
            </div>
            <span className="num w-12 text-right">{navUSD ? ((v / navUSD) * 100).toFixed(1) : 0}%</span>
          </div>
        ))}
      </div>
      {stats && <p className="t-caption">Computed on {stats.obs} daily observations. Statistics firm up as history accumulates.</p>}
    </>
  );
}

/* ---------------- Attribution ---------------- */
export function Attribution({ valued, navUSD, fmt, onAddTrade }: {
  valued: Valued[]; navUSD: number; fmt: (usd: number, dp?: number) => string; onAddTrade: () => void;
}) {
  const { rows, byClass } = useMemo(() => contributions(valued, navUSD), [valued, navUSD]);
  const maxAbs = Math.max(...rows.map(r => Math.abs(r.pnlUSD)), 1);
  if (!rows.length) return <EmptyState text="Attribution builds from your P&L. Log a trade to see what is driving returns." actionLabel="+ Trade" onAction={onAddTrade} />;
  return (
    <>
      <div className="card">
        <h2 className="t-title mb-1">Contribution to return, by position</h2>
        <p className="t-caption mb-3">Each position's total P&L (realized + unrealized) as a share of NAV. Contribution ≈ weight × return, so a small position must work much harder to matter.</p>
      ...
      <div className="card">
        <h2 className="t-title mb-2">By asset class</h2>
        {Object.entries(byClass).sort((a, b) => b[1] - a[1]).map(([k, v]) => (
          <div key={k} className="flex text-[13px] py-2 hair first:border-0">
...
/* ---------------- Rebalance ---------------- */
export function Rebalance({ valued, cashUSD, navUSD, targets, setTargets, fmt, onAddTrade }: {
  valued: Valued[]; cashUSD: number; navUSD: number; targets: Record<string, number>;
  setTargets: (t: Record<string, number>) => void; fmt: (usd: number, dp?: number) => string; onAddTrade: () => void;
}) {
  ...
  if (!valued.filter(v => v.qty > 1e-9).length) return <EmptyState text="Rebalancing needs positions to weigh. Log a trade to set policy targets." actionLabel="+ Trade" onAction={onAddTrade} />;
  return (
    <>
      <div className="card">
        <h2 className="t-title mb-1">Targets vs actual</h2>
        <p className="t-caption mb-3">Set policy weights. Drift beyond ±3% is highlighted; the trade column shows the notional to restore target. Targets sum to <span className={`num ${Math.abs(sumT - 100) < 0.01 ? "text-gain" : "text-brass"}`}>{sumT.toFixed(0)}%</span>.</p>
        <div className="space-y-2">
          {rows.map(r => {
      ...
      <p className="t-caption px-1">Columns: target input · actual weight · drift (pp) · suggested trade. Rebalancing is where discipline beats forecasting: you are systematically selling what ran and buying what lagged.</p>
    </>
  );
}
...
export function Journal({ state, setJournal, setDecisions, valued }: {
  ...
  return (
    <>
      {symbols.map(sym => {
        ...
        return (
          <details key={sym} className="card">
            <summary className="t-title cursor-pointer flex items-center">
              <span className="num">{sym}</span>
              <span className="ml-auto t-label">{j?.updated ? `updated ${j.updated}` : "no thesis yet"}</span>
            </summary>
            <label className="block t-label mt-3 mb-1">Why I own it</label>
            ...
            <label className="block t-label mt-3 mb-1">Sell triggers</label>
      ...
      {symbols.length === 0 && <p className="t-caption text-center py-4">Theses attach to open positions. Log a trade to begin one.</p>}
      <div className="card">
        <h2 className="t-title mb-2">Decision log</h2>
        <div className="flex gap-2 mb-3">
          ...
          <button onClick={() => { if (!note.trim()) return;
            ...
            setNote(""); }}
            className="press bg-brass text-ink font-semibold rounded-xl px-3 text-sm shrink-0">Log</button>
        </div>
        {state.decisions.map(d => (
          <div key={d.id} className="text-[13px] py-2 hair first:border-0">
